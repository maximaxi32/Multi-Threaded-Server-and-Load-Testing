...
void *user_function(void *arg) {
  ...
  while (1) {
    ...
    bzero((char *)&serv_addr, sizeof(serv_addr)); //initializing
    serv_addr.sin_family = AF_INET;
    bcopy((char *)server->h_addr, (char *)&serv_addr.sin_addr.s_addr,
    server->h_length);
    serv_addr.sin_port = htons(info->portno); //port number assigned
    ...
    int temp = rand()%3;   //send random requests out of the three above requests
    // n = write(sockfd, msgs[temp], strlen(msgs[temp]));
...
int main(int argc, char *argv[]) {
  ...
  Avg_throughput = (float)Avg_throughput/test_duration;
